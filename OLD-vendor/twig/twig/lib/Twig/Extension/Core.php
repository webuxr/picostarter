...
class Twig_Extension_Core extends Twig_Extension
{
    ...
    public function getFilters()
    {
        $filters = [
            ...
            //new Twig_SimpleFilter('time', 'twig_time_format_filter', ['needs_environment' => true]),
...
/**
 * Converts a time to the given format (%I:%M:%S %p = 04:04:11 PM).
 *
 * <pre>
 *   {{ post.published_at|time("%I:%M:%S %p") }}
 * </pre>
 *
 * @param Twig_Environment                               $env
 * @param DateTime|DateTimeInterface|DateInterval|string $date     A date
 ...
 * @return string The formatted date
 */
/*function twig_time_format_filter(Twig_Environment $env, $time, $format = null, $timezone = null)
{
    if (null === $format) {
        $formats = $env->getExtension('Twig_Extension_Core')->getTimeFormat();
        $format = $time instanceof TimeInterval ? $formats[1] : $formats[0];
    }
    ...
    return twig_time_converter($env, $time, $timezone)->format($format);
}*/

/**
 * Returns a new time object modified.
 *
 * <pre>
 *   {{ post.published_at|time_modify("-1day")|time("I:M p") }}
 * </pre>
 *
 * @param Twig_Environment $env
 * @param DateTime|string  $time     A time
 * @param string           $modifier A modifier string
 *
 * @return DateTime A new time object
 */
/*function twig_time_modify_filter(Twig_Environment $env, $dtime, $modifier)
{
    $time = twig_time_converter($env, $time, false);
    $resultTime = $time->modify($modifier);

    // This is a hack to ensure PHP 5.2 support and support for DateTimeImmutable
    // DateTime::modify does not return the modified DateTime object < 5.3.0
    // and DateTimeImmutable does not modify $date.
    return null === $resultTime ? $time : $resultTime;
}*/
 ...
 *
 * <pre>
 *    {% if time(user.created_at) < time('+2days') %}
 *      {# do something #}
 *    {% endif %}
 * </pre>
 *
 * @param Twig_Environment                       $env
 * @param DateTime|DateTimeInterface|string|null $time     A date
 * @param DateTimeZone|string|false|null         $timezone The target timezone, null to use the default, false to leave unchanged
 *
 * @return DateTime A DateTime instance
 */
/*function twig_time_converter(Twig_Environment $env, $time = null, $timezone = null)
{
    // determine the timezone
    if (false !== $timezone) {
        if (null === $timezone) {
    ...
    if ($time instanceof DateTimeImmutable) {
        return false !== $timezone ? $time->setTimezone($timezone) : $time;
    }
    ...
    if (null === $time || 'now' === $time) {
        return new DateTime($time, false !== $timezone ? $timezone : $env->getExtension('Twig_Extension_Core')->getTimezone());
    }

    $asString = (string) $time;
    if (ctype_digit($asString) || (!empty($asString) && '-' === $asString[0] && ctype_digit(substr($asString, 1)))) {
        $time = new DateTime('@'.$time);
    } else {
        $time = new DateTime($time, $env->getExtension('Twig_Extension_Core')->getTimezone());
    }
